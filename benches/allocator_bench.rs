...
fn benchmark_pool_allocator(c: &mut Criterion) {
    ...
    for block_size in [64, 256, 1024, 4096].iter() {
        group.bench_with_input(BenchmarkId::new("allocate_deallocate", block_size), block_size, |b, &block_size| {
            let mut memory = vec![0u8; 1024 * 1024]; // 1MB
...
fn benchmark_buffer_pool(c: &mut Criterion) {
    ...
    for buffer_size in [1024, 4096, 16384].iter() {
        group.bench_with_input(BenchmarkId::new("get_return", buffer_size), buffer_size, |b, &buffer_size| {
            let manager = SharedMemoryManager::new();
            let region_config = RegionConfig {
                name: format!("bench_region_{}", buffer_size),
                size: 64 * 1024 * 1024, // 64MB
                backing_type: BackingType::FileBacked,
                file_path: Some(std::path::PathBuf::from(format!("/tmp/renoir_bench_{}", buffer_size))),
                create: true,
                permissions: 0o644,
            };
...
fn benchmark_ring_buffer(c: &mut Criterion) {
    ...
    for capacity in [256, 1024, 4096].iter() {
        group.bench_with_input(BenchmarkId::new("push_pop", capacity), capacity, |b, &capacity| {
...
fn benchmark_sequenced_ring_buffer(c: &mut Criterion) {
    ...
    for capacity in [256, 1024, 4096].iter() {
        group.bench_with_input(BenchmarkId::new("claim_read", capacity), capacity, |b, &capacity| {
...
fn benchmark_memory_copy(c: &mut Criterion) {
    ...
    for size in [1024, 4096, 16384, 65536].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("buffer_write_read", size), size, |b, &size| {
            let manager = SharedMemoryManager::new();
            let region_config = RegionConfig {
                name: format!("memcpy_region_{}", size),
                size: 128 * 1024 * 1024, // 128MB
                backing_type: BackingType::FileBacked,
                file_path: Some(std::path::PathBuf::from(format!("/tmp/renoir_memcpy_{}", size))),
                create: true,
                permissions: 0o644,
            };
