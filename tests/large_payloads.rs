use renoir::large_payloads::{
    blob::{BlobHeader, BlobManager, BlobDescriptor, content_types},
    chunking::ChunkingStrategy,
    reclamation::ReclamationPolicy,
    ros2_types::{ROS2MessageType, ImageHeader, PointCloudHeader, LaserScanHeader},
};
use renoir::{
...
pub fn create_test_pool_manager() -> Option<(Arc<SharedBufferPoolManager>, renoir::shared_pools::PoolId)> {
    ...
    let region = Arc::new(SharedMemoryRegion::new(RegionConfig {
        name: test_name,
...
mod ros2_types_tests {
    ...
    fn test_ros2_message_types() {
        ...
        assert_eq!(ROS2MessageType::PointCloud2.content_type(), content_types::POINT_CLOUD);
        assert_eq!(ROS2MessageType::LaserScan.content_type(), content_types::LASER_SCAN);
