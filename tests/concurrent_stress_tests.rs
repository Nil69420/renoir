...
use std::{
    sync::{Arc, atomic::{AtomicUsize, AtomicBool, Ordering}, Mutex, Barrier},
    thread,
...
mod concurrent_stress_tests {
    ...
    fn stress_maximum_thread_contention() {
        ...
        println!("Max contention: {} threads, {} operations, {} errors, {:?} time, {:.0} ops/sec",
    ...
    fn stress_topic_ring_race_conditions() {
        ...
        println!("Race condition test: {} producers, {} consumers, {} sent, {} received in {:?}",
                producer_count, consumer_count, sent, received, total_time);
        println!("Unique sent IDs: {}, unique received IDs: {}", sent_ids.len(), recv_ids.len());
        
        assert_eq!(sent_ids.len(), sent, "All sent messages should have unique IDs");
        assert_eq!(recv_ids.len(), received, "All received messages should have unique IDs");
        assert!(received >= sent / 2, "Should receive at least half the sent messages");
    }
    ...
    fn stress_buffer_pool_high_contention() {
        ...
        let success_rate = total_successful as f64 / (total_successful + total_failures) as f64 * 100.0;
        ...
        assert!(success_rate > 50.0, "Should maintain >50% success rate under contention");
        assert!(total_successful > thread_count * allocations_per_thread / 4, 
    ...
    fn stress_memory_region_access_storm() {
        ...
        println!("Access storm: {} threads, {} operations in {:?} ({:.0} ops/sec)",
                thread_count, total_operations, storm_duration, ops_per_second);
        
        assert!(total_operations > thread_count * accesses_per_thread / 2,
               "Should complete majority of operations in storm");
        assert!(ops_per_second > 5000.0, "Should maintain high throughput under storm");
    }
    ...
    fn stress_deadlock_prevention() {
        ...
        println!("Deadlock prevention: {} threads completed, {} operations in {:?}",
                completed_threads, total_operations, test_duration);
        
        assert_eq!(completed_threads, thread_count, "All threads should complete without deadlock");
        assert!(test_duration < timeout_duration, "Should complete within reasonable time");
    ...
    fn stress_high_frequency_create_destroy() {
        ...
        println!("High-frequency create/destroy: {} created, {} destroyed in {:?} ({:.0} ops/sec)",
        ...
        assert!(created > thread_count, "Should successfully create some regions");
        assert!(destroyed >= created / 2, "Should destroy most created regions");
    }
}