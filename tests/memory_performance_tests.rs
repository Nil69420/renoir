...
mod memory_performance_tests {
    ...
    fn perf_memory_allocation_speed() {
        ...
        println!(
            "Allocated {} buffers in {:?} ({:.2} allocs/ms)",
        ...
        println!(
            "Deallocated {} buffers in {:?}",
            successful_allocations, deallocation_time
        );

        // Verify performance thresholds (embedded-friendly)
        assert!(
            successful_allocations > 100,
            "Should allocate at least 100 buffers"
        );
        assert!(
            allocation_time.as_millis() < 2000,
            "Allocation should complete within 2 seconds"
        );
    }
    ...
    fn perf_fragmentation_resistance() {
        ...
        println!(
            "Fragmentation test: {} regions created, {} final count, {:?} total time",
        ...
        assert!(
            fragmentation_time.as_secs() < 5,
            "Fragmentation handling should be fast"
        );
        ...
        for &size in &large_sizes {
            ...
            match manager.create_region(config) {
                Ok(_region) => {
                    ...
                    assert!(
                        creation_time.as_millis() < max_time_ms as u128,
                        "Large region creation too slow: {:?} for {:.1}MB",
                        creation_time,
                        size_mb
                    );
                }
    ...
    fn perf_concurrent_allocation_throughput() {
        ...
        println!(
            "Concurrent allocation: {} allocations by {} threads in {:?} ({:.2} allocs/sec)",
            total_allocated, thread_count, total_time, throughput
        );

        assert!(
            total_allocated > thread_count * allocations_per_thread / 2,
            "Should succeed on at least half the allocations"
        );
        assert!(
            throughput > 100.0,
            "Should achieve at least 100 allocations/sec"
        );
    }
    ...
    fn perf_memory_access_patterns() {
        ...
        for buffer in &mut buffers {
            ...
            for i in (0..len).step_by(37) {
                // Prime step for pseudo-randomness
                slice[i] = (i % 256) as u8;
            }
        }
        ...
        assert!(
            sequential_time.as_millis() < 2000,
            "Sequential access should be fast"
        );
        assert!(
            random_time.as_millis() < 5000,
            "Random access should be reasonably fast"
        );
    }
    ...
    fn perf_memory_cleanup_speed() {
        ...
        println!(
            "Created {} regions in {:?}, cleaned up in {:?}",
        ...
        assert!(
            creation_time.as_millis() < 30000,
            "Mass creation should complete in 30s"
        );
        assert!(
            cleanup_time.as_millis() < 15000,
            "Mass cleanup should complete in 15s"
        );
    }
    ...
    fn perf_allocator_comparison() {
        ...
        println!(
            "Pool allocator: {:?} alloc + {:?} dealloc",
            pool_alloc_time, pool_dealloc_time
        );

        // Both should be reasonably fast on embedded systems
        assert!(
            bump_time.as_millis() < 500,
            "Bump allocator should be reasonably fast"
        );
        assert!(
            pool_alloc_time.as_millis() < 1000,
            "Pool allocator should be reasonably fast"
        );
        assert!(
            pool_dealloc_time.as_millis() < 500,
            "Pool deallocation should be reasonably fast"
        );
    }
    ...
    fn perf_memory_pressure_recovery() {
        ...
        for _ in 0..std::cmp::min(release_count, 20) {
        ...
        println!(
            "Memory pressure test: filled {} buffers in {:?}, recovered {} in {:?}",
