...
use renoir::{
    ...
    message_formats::{
        FormatType,
        registry::{SchemaInfo, ZeroCopyFormatRegistry, UseCase},
        traits::ZeroCopyBuilder,
        zero_copy::{FlatBufferFormat, CapnProtoFormat, create_sensor_data_flatbuffer, create_pose_data_capnp},
    },
};
...
mod zero_copy_integration_tests {
    ...
    fn test_custom_message_building() -> Result<()> {
        ...
        assert_eq!(accessor.get_string("device_name")?.as_deref().unwrap_or(""), "IMU_01");
