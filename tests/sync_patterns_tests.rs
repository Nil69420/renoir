...
use std::{
    sync::{Arc, atomic::{AtomicUsize, Ordering}, Barrier, Mutex},
    thread,
    time::{Duration, Instant},
};

use renoir::sync::{
    sequence::{AtomicSequence, SequenceChecker, GlobalSequenceGenerator, special, next_local_sequence},
    swmr::SWMRRing,
...
mod sync_patterns_tests {
    ...
    fn test_swmr_multiple_readers() {
        ...
        let reader_handles: Vec<_> = (0..3).map(|reader_id| {
        ...
        for (reader_id, data) in results.iter() {
            ...
            assert!(!data.is_empty(), "Reader {} should have read some data", reader_id);
    ...
    fn test_epoch_with_readers() {
        ...
        let reader_handles: Vec<_> = (0..2).map(|_reader_id| {
    ...
    fn test_swmr_with_notifications() {
        ...
        for (i, msg) in received.iter().enumerate().take(5) {
            assert!(msg.starts_with("Message"), "Message {} should start with 'Message'", i);
        }
    }
    ...
    fn test_local_sequence_performance() {
        ...
        assert!(local_time.as_millis() < 100, "Local sequence should be fast");
        assert!(global_time.as_millis() < 100, "Global sequence should be fast");
    }
    ...
    fn test_integrated_patterns() {
        ...
        let consumer_handle = thread::spawn(move || {
            ...
            for _ in 0..25 { // Try to read more than produced
