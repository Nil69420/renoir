...
use renoir::{
    error::Result,
    message_formats::{
        registry::{UseCase, ZeroCopyFormatRegistry},
...
mod performance_tests {
    ...
    fn test_message_creation_performance() -> Result<()> {
        ...
        println!(
            "Message creation: {}ns avg, {:.0} msg/sec",
            avg_time_ns, throughput
        );
    ...
    fn test_high_frequency_simulation() -> Result<()> {
        ...
        println!(
            "High frequency test: {:.0}Hz actual ({:.1}% efficiency)",
    ...
    fn test_sensor_fusion_throughput() -> Result<()> {
        ...
        println!(
            "Sensor fusion - {:.0} msg/sec, {:.1} KB/s",
            throughput, data_rate_kb
        );
    ...
    fn test_memory_efficiency() -> Result<()> {
        ...
        for i in 0..message_count {
            let accessor = create_sensor_data_flatbuffer(i as u64, i as f64, i as u32)?;
    ...
    fn test_concurrent_access_simulation() -> Result<()> {
        ...
        println!(
            "Concurrent access simulation: {}ns avg per field",
            avg_time_ns
        );
    ...
    fn test_zero_copy_validation() -> Result<()> {
        ...
        let accessors: Vec<_> = (0..100)
            .map(|i| create_sensor_data_flatbuffer(i, i as f64, i as u32))
        ...
        println!(
            "Zero-copy validation: {} messages verified",
            accessors.len()
        );
