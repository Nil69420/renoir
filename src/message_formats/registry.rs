...
impl ZeroCopyFormatRegistry {
    ...
    fn register_default_formats(&mut self) {
        ...
        self.formats.insert(FormatType::FlatBuffers, Arc::new(FlatBufferFormat::new()));
        self.formats.insert(FormatType::CapnProto, Arc::new(CapnProtoFormat::new()));
    }
    ...
    pub fn get_format(&self, format_type: &FormatType) -> Option<Arc<dyn ZeroCopyFormat + Send + Sync>> {
    ...
    pub fn assign_topic_format(&mut self, topic_pattern: String, format_type: FormatType) -> Result<()> {
        if !self.formats.contains_key(&format_type) {
            return Err(RenoirError::invalid_parameter(
                "format_type",
...
pub fn register_global_schema(schema: SchemaInfo) -> Result<()> {
    ...
    let mut registry = registry.write().map_err(|_| RenoirError::memory("Registry lock poisoned".to_string()))?;
    registry.register_schema(schema)
}

/// Get a format from the global registry
pub fn get_global_format(format_type: &FormatType) -> Option<Arc<dyn ZeroCopyFormat + Send + Sync>> {
    let registry = global_registry();
