...
use super::schema_evolution::{EvolutionAwareSchema, FieldDefinition, CompatibilityLevel, FieldChange};
    ...
    pub fn validate_backward_compatibility(
        ...
        for (field_name, old_field) in &old_schema.fields {
            if let Some(new_field) = new_schema.fields.get(field_name) {
                ...
                Self::validate_field_compatibility(old_field, new_field, &mut violations, &mut warnings)?;
            } else {
        ...
        Self::validate_format_specific_backward_compatibility(
            old_schema, new_schema, &mut violations, &mut warnings
        )?;
        ...
        for (field_name, old_field) in &old_schema.fields {
            if let Some(new_field) = new_schema.fields.get(field_name) {
                ...
                if old_field.field_type != new_field.field_type {
                    if !Self::are_types_forward_compatible(&old_field.field_type, &new_field.field_type) {
                        violations.push(format!(
        ...
        let compatibility_level = if violations.is_empty() {
            if warnings.is_empty() {
                ...
                match (backward.compatibility_level, forward.compatibility_level) {
                    (CompatibilityLevel::FullCompatible, CompatibilityLevel::FullCompatible) => CompatibilityLevel::FullCompatible,
                    (CompatibilityLevel::BackwardCompatible, _) | (_, CompatibilityLevel::BackwardCompatible) => CompatibilityLevel::BackwardCompatible,
                    (CompatibilityLevel::ForwardCompatible, _) | (_, CompatibilityLevel::ForwardCompatible) => CompatibilityLevel::ForwardCompatible,
    ...
    pub fn analyze_field_changes(
        ...
        for (field_name, old_field) in &old_schema.fields {
            if let Some(new_field) = new_schema.fields.get(field_name) {
                // Field type changed
                if old_field.field_type != new_field.field_type {
                    let is_compatible = Self::are_types_backward_compatible(&old_field.field_type, &new_field.field_type);
    ...
    fn validate_format_specific_backward_compatibility(
        ...
        match old_schema.base.format_type {
            FormatType::FlatBuffers => {
                Self::validate_flatbuffers_backward_compatibility(old_schema, new_schema, violations, warnings)
            }
            FormatType::CapnProto => {
                Self::validate_capnproto_backward_compatibility(old_schema, new_schema, violations, warnings)
            }
            FormatType::Custom => {
                // Custom formats need custom validation
                warnings.push("Custom format compatibility cannot be automatically validated".to_string());
                Ok(())
            }
        }
    }
...
mod tests {
    ...
    fn test_backward_compatibility_optional_field_added() -> Result<()> {
        ...
        let old_schema = SchemaBuilder::new("test".to_string(), FormatType::FlatBuffers)
            .version(1, 0, 0)
            .add_field(FieldDefinition::new("field1".to_string(), "u32".to_string(), true, 1, 0))
            .build(&mut manager)?;
        ...
        let new_schema = SchemaBuilder::new("test".to_string(), FormatType::FlatBuffers)
            ...
            .version(1, 1, 0)
            .add_field(FieldDefinition::new("field1".to_string(), "u32".to_string(), true, 1, 0))
            .add_field(FieldDefinition::new("field2".to_string(), "string".to_string(), false, 2, 1))
            .build(&mut manager)?;

        let result = SchemaCompatibilityValidator::validate_backward_compatibility(&old_schema, &new_schema)?;
        
        assert!(result.is_compatible);
        assert_eq!(result.compatibility_level, CompatibilityLevel::FullCompatible);
    ...
    fn test_backward_compatibility_required_field_added() -> Result<()> {
        ...
        let new_schema = SchemaBuilder::new("test".to_string(), FormatType::FlatBuffers)
            ...
            .version(2, 0, 0)
            .add_field(FieldDefinition::new("field1".to_string(), "u32".to_string(), true, 1, 0))
            .add_field(FieldDefinition::new("field2".to_string(), "string".to_string(), true, 2, 1))
            .build(&mut manager)?;
    ...
    fn test_type_compatibility() {
        assert!(SchemaCompatibilityValidator::are_types_backward_compatible("u32", "u64"));
        assert!(!SchemaCompatibilityValidator::are_types_backward_compatible("u64", "u32"));
        assert!(SchemaCompatibilityValidator::are_types_backward_compatible("f32", "f64"));
        assert!(!SchemaCompatibilityValidator::are_types_forward_compatible("u64", "u32"));
    }
}