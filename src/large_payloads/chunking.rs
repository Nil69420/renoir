...
impl IncompleteMessage {
    ...
    fn add_chunk(&mut self, chunk_data: Vec<u8>, descriptor: ChunkDescriptor) -> Result<()> {
        ...
        self.chunks.insert(descriptor.chunk_index, (chunk_data, descriptor));
        Ok(())
    }
    ...
    fn reassemble(&self) -> Result<Vec<u8>> {
        ...
        let total_size: usize = self.chunks.values()
            .map(|(data, _)| data.len())
            .sum();
...
impl ChunkManager {
    ...
    pub fn chunk_payload(
        ...
        let chunk_size = match &self.strategy {
            ...
            ChunkingStrategy::Adaptive { min_size, max_size: _ } => {
    ...
    pub fn process_chunk(&self, chunk_buffer: &[u8]) -> Result<Option<Vec<u8>>> {
        ...
        let chunk_desc: ChunkDescriptor = unsafe {
            std::ptr::read_unaligned(chunk_buffer.as_ptr() as *const ChunkDescriptor)
        };
        
        // Extract chunk data
        let chunk_data_offset = ChunkDescriptor::SIZE;
        let chunk_data = &chunk_buffer[chunk_data_offset..chunk_data_offset + chunk_desc.chunk_size as usize];
    ...
    pub fn get_statistics(&self) -> ChunkManagerStats {
        ...
        let total_chunks_waiting: u32 = incomplete.values()
            .map(|msg| msg.chunks.len() as u32)
            .sum();
        let oldest_message_age = incomplete.values()
