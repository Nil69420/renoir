...
impl StructuredMemoryRegion {
    ...
    pub fn create(region: Arc<SharedMemoryRegion>, initialize: bool) -> Result<Self> {
        ...
        if initialize {
            ...
            let topic_directory =
                unsafe { base_ptr.add(size_of::<GlobalControlHeader>()) as *mut TopicDirectory };
            unsafe {
    ...
    pub fn find_topic(&self, name: &str) -> Option<&TopicDirectoryEntry> {
        ...
        for i in 0..directory.entry_count as usize {
            ...
            if entry_name_len == name_bytes.len() && &entry.name[..entry_name_len] == name_bytes {
                return Some(entry);
            }
        }
    ...
    pub fn get_topic_header(&self, topic_offset: u32) -> Result<*mut TopicControlHeader> {
        ...
        let topic_header =
            unsafe { base_ptr.add(topic_offset as usize) as *mut TopicControlHeader };
        Ok(topic_header)
    }
}
