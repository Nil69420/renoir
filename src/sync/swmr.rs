...
use super::{
    sequence::{AtomicSequence, SequenceNumber, SequenceChecker, special},
...
impl<'a, T: Clone> ReaderHandle<'a, T> {
    ...
    pub fn try_read(&mut self) -> SyncResult<Option<(T, SequenceNumber)>> {
        ...
        match checker.consistent_read(|| {
            unsafe { (*slot.data_ptr()).clone() }
        }, 3) {
            Ok(data) => {
...
mod tests {
    ...
    fn test_swmr_multiple_readers() {
        ...
        let reader_handles: Vec<_> = (0..2).map(|reader_id| {
            ...
            thread::spawn(move || {
                ...
                for _ in 0..20 { // Try to read more than writer produces
        ...
        let results: Vec<_> = reader_handles.into_iter()
            .map(|h| h.join().unwrap())
            .collect();
