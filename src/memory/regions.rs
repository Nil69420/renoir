...
impl SharedMemoryRegion {
    ...
    pub fn new(config: RegionConfig) -> Result<Self> {
        ...
        let (file, fd) = Self::create_backing(&config)?;
        let mmap = Self::create_mapping(&file, fd, config.size)?;
    ...
    fn create_backing(config: &RegionConfig) -> Result<(Option<File>, RawFd)> {
    ...
    fn create_file_backing(config: &RegionConfig) -> Result<(Option<File>, RawFd)> {
    ...
    #[cfg(target_os = "linux")]
    fn create_memfd_backing(config: &RegionConfig) -> Result<(Option<File>, RawFd)> {
    ...
    fn create_mapping(file: &Option<File>, fd: RawFd, size: usize) -> Result<MmapMut> {
        match file {
            Some(f) => {
